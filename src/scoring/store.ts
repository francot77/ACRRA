...
export class ScoringStore {
  ...
  getReport(reportId: string): ReportOutboxEntry | null {
    const row = this.database.prepare('SELECT report_id, run_id, status, payload_json, attempts, created_at FROM scoring_report_outbox WHERE report_id = ?').get(reportId) as
      | { report_id: string; run_id: string; status: ReportOutboxEntry['status']; payload_json: string; attempts: number; created_at: string }
      | undefined;
    return row ? { reportId: row.report_id, runId: row.run_id, status: row.status, payloadJson: row.payload_json, attempts: row.attempts, createdAt: row.created_at } : null;
  }

  getLatestReport(): ReportOutboxEntry | null {
    const row = this.database.prepare('SELECT report_id, run_id, status, payload_json, attempts, created_at FROM scoring_report_outbox ORDER BY created_at DESC, rowid DESC LIMIT 1').get() as
      | { report_id: string; run_id: string; status: ReportOutboxEntry['status']; payload_json: string; attempts: number; created_at: string }
      | undefined;
    return row ? { reportId: row.report_id, runId: row.run_id, status: row.status, payloadJson: row.payload_json, attempts: row.attempts, createdAt: row.created_at } : null;
  }
  ...
  markReportSent(reportId: string): void {
    this.database.prepare("UPDATE scoring_report_outbox SET status = 'sent', attempts = attempts + 1, sent_at = ?, last_error = NULL WHERE report_id = ?").run(new Date().toISOString(), reportId);
  }
